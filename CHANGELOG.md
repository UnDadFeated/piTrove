...
- **2D Edge Glow with Screen Edge Fade** — Bias lighting now uses 2D falloff from the image border for top/bottom strips, 2D radial gradient from corners for side strips, and a screen-edge fade layer that bridges edge glow strips into corner glow with matching 2D distance falloff.
- **Configurable Blur, Matte, Vignette & Glow** — New settings: `blur_radius` (6–24, default 14), `matte_opacity` (0.05–0.50, default 0.20), `vignette_strength` (0.10–0.80, default 0.35), and `glow_depth` (16–120, default 86). All tuned for natural appearance without heavy GPU cost (~5ms on Pi 4/5).
